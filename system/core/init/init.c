...
static void selinux_initialize(void)
{
    ...
    INFO("loading selinux policy blin...\n");
    if (selinux_android_load_policy() < 0) {
        ERROR("SELinux: Failed to load policy; rebooting into recovery mode. blin.\n");
    ...
    // не будет тебе селинукса
    bool is_enforcing = false; 
    
    INFO("SELinux: security_setenforce(%d) permissive kaka pisun uraaa\n", is_enforcing);
